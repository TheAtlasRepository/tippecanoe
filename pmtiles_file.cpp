...
void mbtiles_map_image_to_pmtiles_stream(char *fname, metadata m, bool tile_compression, bool fquiet, bool fquiet_progress, std::ostream &output_stream) {
	sqlite3 *db;
	...
	// write tile data to tempfile in clustered order
	{
		...
		for (auto const &tile_id : tile_ids) {
			...
			if (hash_to_offset_len.count(hsh) > 0) {
				...
				sqlite3_bind_text(image_stmt, 1, hsh.data(), hsh.size(), SQLITE_STATIC);
				if (sqlite3_step(image_stmt) != SQLITE_ROW) {
					fprintf(stderr, "Corrupt mbtiles file: no tile with id %s\n", hsh.c_str());
					exit(EXIT_SQLITE);
				}

				int len = sqlite3_column_bytes(image_stmt, 0);
				auto data = reinterpret_cast<const char *>(sqlite3_column_blob(image_stmt, 0));

				if (entries.size() > 0 && tile_id == entries[entries.size() - 1].tile_id + 1 && entries[entries.size() - 1].offset == offset) {
					entries[entries.size() - 1].run_length++;
				} else {
					entries.emplace_back(tile_id, offset, len, 1);
				}
	...
	// finalize PMTiles archive to stream.
	{
		std::stable_sort(entries.begin(), entries.end(), pmtiles::entryv3_cmp());
		...
		// Write to provided stream instead of file
		auto header_str = header.serialize();
		output_stream.write(header_str.data(), header_str.length());
		output_stream.write(root_bytes.data(), root_bytes.length());
		output_stream.write(json_metadata.data(), json_metadata.size());
		output_stream.write(leaves_bytes.data(), leaves_bytes.length());
		output_stream << tmp_istream.rdbuf();
